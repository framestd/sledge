    ...
    for i in range(len(_filter)):
        for each in temp:
            if each.endswith(_filter[i]):
                __build(basedir, each, render(os.path.join(basedir, each)))
...
def __build(basedir, filename, response):
...
def __doTabs(context=nullstr, tab=nullstr):
...
def hammer(workspace=os.path.dirname(__file__)):
    ...
    if os.path.isfile(workspace):
        __build(None, os.path.basename(workspace), render(workspace))
        return
    basespace = workspace
